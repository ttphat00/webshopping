    ...
    <div id="particles-js">
        ...
        <div class="dashboard">
            ...
            <div class="container w-100" style="height: max-content;">
                <div class="bg-dark text-white myshop"><h3>Cập nhật hàng hóa</h3></div>
                <div class="w-75" style="margin:auto;">
                <?php
                    require_once('../../database/Connection.php');

                    $sql = "SELECT hanghoa.MSHH,TenHH,MaHinh,TenHinh,QuyCach,Gia,SoLuongHang,loaihanghoa.MaLoaiHang,TenLoaiHang FROM hanghoa,hinhhanghoa,loaihanghoa WHERE hanghoa.MSHH=hinhhanghoa.MSHH AND loaihanghoa.MaLoaiHang=hanghoa.MaLoaiHang AND hanghoa.MSHH=".$_GET['id']."";
                    $result = mysqli_query($conn, $sql);

                    if (mysqli_num_rows($result) > 0) {
                        $row = mysqli_fetch_assoc($result);

                        echo '<form action="UpdateProduct.php?id='.$row['MSHH'].'" method="post" enctype="multipart/form-data">
                            <div class="form-group row">
                                <label for="name" class="col-sm-2 col-form-label">Tên hàng hóa</label>
                                <div class="col-sm-10">
                                    <input type="text" class="form-control" name="name" placeholder="Tên hàng hóa" value="'.$row['TenHH'].'" required>
                                </div>
                            </div>
                            <div class="form-group row">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-10">
                                    <textarea class="form-control" name="description" rows="8" style="resize:none;" required>'.$row['QuyCach'].'</textarea>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="cost" class="col-sm-2 col-form-label">Giá</label>
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" name="cost" placeholder="VD: 100000" pattern="^[0-9]+$" value="'.$row['Gia'].'" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                <label for="quantity" class="col-sm-2 col-form-label">Số lượng</label>
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" name="quantity" placeholder="Tối đa là 99" pattern="^[0-9]{1,2}$" value="'.$row['SoLuongHang'].'" required>
                                </div>
                            </div>
                            <div class="form-group row">
                                ...
                                <div class="col-sm-5">
                                    ...
                                    if (mysqli_num_rows($result2) > 0) {
                                        while($row2 = mysqli_fetch_assoc($result2)) {
                                            $selected = ($row2['MaLoaiHang'] === $row['MaLoaiHang']) ? 'selected' : '';
                                            echo '<option value="'.$row2['MaLoaiHang'].'" '.$selected.'>'.$row2['TenLoaiHang'].'</option>';
                                        }
                                    }
                            ...
                            <div style="margin-left:150px;"><button type="submit" name="submit" class="btn btn-primary">Cập nhật</button></div>
                        </form>
                        <div style="margin-left:150px;"><a href="ShowProduct.php"><button class="btn btn-dark mt-3">Trở về</button></a></div>';
                    }
